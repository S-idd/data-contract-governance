...
COMPOSE_FILE="$ROOT_DIR/docker-compose.sqlite.yml"
ENV_FILE="${DCG_SQLITE_DEMO_ENV_FILE:-$ROOT_DIR/.env.sqlite-demo}"
ENV_TEMPLATE="${DCG_SQLITE_DEMO_ENV_TEMPLATE:-$ROOT_DIR/config/compose.sqlite-demo.env.example}"
PROJECT_NAME="dcg-sqlite-demo"

die() { echo "[dcg-sqlite-demo] ERROR: $*" >&2; exit 1; }
...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  echo "Update its DCG_APP_* values before using outside a local demo."
fi

COMPOSE_ENVIRONMENT="$(compose config --environment)"
for required_setting in DCG_APP_USERNAME DCG_APP_PASSWORD; do
...
echo "Starting main DCG with SQLite metadata in Docker..."
compose up --build -d
...
curl -fsS "$BASE_URL/actuator/health" >/dev/null \
  || { compose logs --tail=120 contract-service; die "Service did not become healthy at $BASE_URL"; }

CREATE_RESPONSE="$(curl -fsS -u "${APP_USER}:${APP_PASSWORD}" -H "Content-Type: application/json" \
  -d '{"contractId":"orders.created","baseVersion":"v1","candidateVersion":"v2","mode":"BACKWARD","commitSha":"sqlite-compose-demo","triggeredBy":"sqlite-compose-demo"}' \
  "$BASE_URL/checks")"
RUN_ID="$(echo "$CREATE_RESPONSE" | sed -n 's/.*"runId"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | head -n 1)"

echo "SQLite DCG demo is ready."
