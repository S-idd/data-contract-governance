...
macOS ARM64 candidate passed native extracted-package acceptance. The previously
reported WSL2 acceptance identifies an earlier archive; rerun acceptance against the
current Linux x64 archive before publication. A manually started inference process from
this exact package is now detected and stopped, including when started with a relative
executable path; unrelated listeners are left untouched.
...
## Regenerated candidate checks — 2026-09-16
...
Trivy 0.72.0 scans of both extracted candidates reported no findings, but discovered
no language-specific or configuration files; this does not establish vulnerability
clearance. macOS ARM64 passed full native extracted-package
acceptance, including authentication, AI outage/recovery, repeated lifecycle, and cleanup
of a manually started Rust process invoked with a relative executable path. An earlier
Linux x64 candidate passed an emulated smoke test, but Rosetta-backed container process
maps cannot validate the current executable-identity check. Linux x64 WSL2 acceptance
remains the final platform-specific technical check for these regenerated bytes.
